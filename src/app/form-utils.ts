...
export function forEachControlIn(form: FormGroup | FormArray) {
  ...
  const composer = {
      ...
      if (controls != null && Array.isArray(controls)) {
        controls.forEach(c => {
          ...
          if ((c as any).controls != null) {
            forEachControlIn(c as FormArray | FormGroup).call(...methods);
          }
        });
      }
      return composer;
    },
    markAsDirtySimultaneouslyWith(c?: AbstractControl| null | undefined) {
      if (c != null) {
    ...
    markAsTouchedSimultaneouslyWith(c: AbstractControl| null | undefined, touchIsChildInitiated?: () => boolean) {
      if (c != null) {
